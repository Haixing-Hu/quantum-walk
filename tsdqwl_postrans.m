%TSDQWL_POSTRANS  Calculates the position transformation.
%
%   TSDQWL_POSTRANS(n) Calculates the position transformation of n-steps 
...
function M_p = tsdqwl_postrans(n)
    ...
    if n < 1
        ...
        % M_p = \sum_{k=1}^{d}(|k-1,0><k,0| + |k,1><k,1| + |k+1,1><k,2|)
        for k = 1 : d                           
            ...
            M_p = M_p + ketbra(kron(p_l, c_0), kron(p_o, c_0)) ...
                      ...
                      + ketbra(kron(p_r, c_1), kron(p_o, c_2));        
        end   
    end
end       