%QUANTUM_WALK_PERIODICAL_WAVE Generates the animation of the probability 
%                   wave of continuous-time quantum walks on a periodical
%                   line segment.
%
%  QUANTUM_WALK_PERIODICAL_WAVE(n, x, T) Generates the animation of the 
%                   probability wave of continuous-time quantum walks on a 
...
function quantum_walk_periodical_wave(n, x, T)
    close all; clc;
    ...
    for t = 0.0 : precision : T
        count = count + 1;        
        psi{count,1} = continuous_quantum_walk(t, A, psi_0);               
    end        
